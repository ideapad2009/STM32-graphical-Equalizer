/*
 * body_DFSDM_I2S_CMSIS_df1.h
 *
 *  Created on: May 16, 2020
 *      Author: Hanna Nabil
 */
/*   DFSDM--> I2S  CMSIS  df1 Direct Form I structure  mono    */
#ifndef INC_BODY_DF1_MONO_H_
#define INC_BODY_DF1_MONO_H_
...
arm_biquad_casd_df1_inst_f32 S1, S2, S3,S4,S5;
...
int main(void)
{
	...
	arm_biquad_cascade_df1_init_f32(&S1, NUMSTAGES,&band1_coeff[ (5*NUMSTAGES)*(gainDB[0]+MAX_GAIN)],&biquadStateBand1[0]);
	arm_biquad_cascade_df1_init_f32(&S2, NUMSTAGES,&band2_coeff[ (5*NUMSTAGES)*(gainDB[1]+MAX_GAIN)],&biquadStateBand2[0]);
	arm_biquad_cascade_df1_init_f32(&S3, NUMSTAGES,&band3_coeff[ (5*NUMSTAGES)*(gainDB[2]+MAX_GAIN)],&biquadStateBand3[0]);
	arm_biquad_cascade_df1_init_f32(&S4, NUMSTAGES,&band4_coeff[ (5*NUMSTAGES)*(gainDB[3]+MAX_GAIN)],&biquadStateBand4[0]);
	arm_biquad_cascade_df1_init_f32(&S5, NUMSTAGES,&band5_coeff[ (5*NUMSTAGES)*(gainDB[4]+MAX_GAIN)],&biquadStateBand5[0]);
	...
	while (1)
	{
		if((DmaRightRecHalfBuffCplt == 1))
		{
			...
			arm_biquad_cascade_df1_f32(&S1, (float32_t *)&r_buf_in[0], &r_buf_out[0],AUDIO_REC/2);
			arm_biquad_cascade_df1_f32(&S2, &r_buf_out[0],&r_buf_out[0],AUDIO_REC/2);
			arm_biquad_cascade_df1_f32(&S3, &r_buf_out[0],&r_buf_out[0],AUDIO_REC/2);
			arm_biquad_cascade_df1_f32(&S4, &r_buf_out[0],&r_buf_out[0],AUDIO_REC/2);
			arm_biquad_cascade_df1_f32(&S5, &r_buf_out[0],&r_buf_out[0],AUDIO_REC/2);
			for(i = 0; i < AUDIO_REC/2; i++)
			{
		...
		if( (DmaRightRecBuffCplt == 1))
		{
			...
			arm_biquad_cascade_df1_f32(&S1,(float32_t *)&r_buf_in[AUDIO_REC/2] , &r_buf_out[AUDIO_REC/2],AUDIO_REC/2);
			arm_biquad_cascade_df1_f32(&S2, &r_buf_out[AUDIO_REC/2],&r_buf_out[AUDIO_REC/2],AUDIO_REC/2);
			arm_biquad_cascade_df1_f32(&S3, &r_buf_out[AUDIO_REC/2],&r_buf_out[AUDIO_REC/2],AUDIO_REC/2);
			arm_biquad_cascade_df1_f32(&S4, &r_buf_out[AUDIO_REC/2],&r_buf_out[AUDIO_REC/2],AUDIO_REC/2);
			arm_biquad_cascade_df1_f32(&S5, &r_buf_out[AUDIO_REC/2],&r_buf_out[AUDIO_REC/2],AUDIO_REC/2);
			for(i = AUDIO_REC/2; i < AUDIO_REC; i++)
			{
				txBuf[i*4]  =  ((int)r_buf_out[i])>>16;
				txBuf[(i*4)+1]= ((int)r_buf_out[i])&0xFFFF;
